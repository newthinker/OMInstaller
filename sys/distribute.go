...
// 进行分布式安装
func Distribute(basedir string, sc *SysConfig, sm *ServerMapping) error {
    // 首先判断SysInfo.xml文件是否存在
    filepath := basedir + "/conf/" + SYS_INFO
    si, err := OpenSIConfig(filepath)
    if err!=nil {
        fmt.Println("打开SysInfo.xml文件失败")
        return err
    }
